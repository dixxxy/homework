...
def get_email_cntent(message, base_save_path):
    ...
    for part in message.walk():
        ...
        if file_name:  # Attachment
            ...
            try:
                att_file = open(base_save_path + filename, 'wb')  # 注意一定要用wb来打开文件，因为附件一般都是二进制文件
                attachment_files.append(filename)
            except:
                print '附件名有非法字符，自动换一个'
                att_file = open('aaaa', 'wb')
                attachment_files.append(filename)
            att_file.write(data)
            att_file.close()
...
if __name__ == '__main__':
    # 输入邮件地址, 口令和POP3服务器地址:
    emailaddress = 'duanxiaoyang@cnnic.cn'
    # 注意使用开通POP，SMTP等的授权码
    password = '101010dxya.'
    pop3_server = 'pop.cnnic.cn'
    ...
    for i in range(length):
        ...
        cur_dir = r'G:/GetFile/cnnic/'
        new_title = title.split('<')[0]
        final_title = new_title.split('@')[0]
